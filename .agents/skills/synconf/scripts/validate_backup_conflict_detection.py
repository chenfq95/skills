#!/usr/bin/env python3
"""Validate that backup conflict detection runs before write decisions."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="synconf-backup-conflicts-") as temp_dir:
        root = Path(temp_dir)
        home_dir = root / "home"
        repo_dir = root / "repo"

        write_file(home_dir / ".zshrc", "export PATH=$HOME/bin\n")
        write_file(repo_dir / "shell/.zshrc", "export PATH=/old/home/bin\n")
        ...
        conflicts = backup.collect_conflicts(
            entries,
            home_dir=home_dir,
            repo_dir=repo_dir,
        )
        ...
        if conflict["source"] != str(home_dir / ".zshrc"):
            raise AssertionError("Unexpected conflict source: " + conflict["source"])
        if conflict["target"] != str(repo_dir / "shell/.zshrc"):
            raise AssertionError("Unexpected conflict target: " + conflict["target"])
        ...
        resolved = backup.resolve_conflict_action(
            entries[0],
            "skip",
            decisions,
        )
        ...
        fallback = backup.resolve_conflict_action(
            entries[1],
            "skip",
            decisions,
        )
    ...
    print("Backup conflict detection validation passed.")
